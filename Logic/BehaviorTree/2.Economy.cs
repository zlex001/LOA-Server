...
namespace Logic.BehaviorTree
{
    ...
    public class Economy
    {
        ...
        {
            var item = character as global::Data.Item;
        ...
        private static bool IsGarbage(Character character)
        {
            var item = character as global::Data.Item;
            if (item == null) return false;
            if (item.Config.Id == global::Data.Constant.Money) return false;
        ...
        public static bool HasClearanceTarget(Character character)
        {
            var life = character as global::Data.Life;
            if (life == null) return false;
            List<Item> garbage = Logic.Exchange.Agent.GetItemsInBag(life, IsGarbage);
        ...
        public static bool HasProductInBag(Character character)
        {
            var life = character as global::Data.Life;
            if (life == null) return false;
            List<Character> characters = Logic.Exchange.Give.Availability(life);
        ...
        public static bool ClearanceToMerchant(Character character)
        {
            ...
            if (life == null) return false;
            if (life.Map == null) return false;
            var owner = life.Map.Content.Get<Life>(l => Logic.Infrastructure.Agent.Guild.IsOwner(l));
        ...
        {
            if (character is not global::Data.Life life) return false;
            Item box = Infrastructure.Agent.GetBox(life.Map, Infrastructure.ContainerType.Material);
            if (box == null) return false;
            ...
            foreach (Character c in Exchange.Give.Availability(life))
            {
                if (IsMaterial(c))
                {
                    var item = c as global::Data.Item;
        ...
        public static bool SelfSupply(Character character)
        {
            ...
            foreach (Item box in Infrastructure.Agent.GetBoxes(life.Map))
            {
                foreach (Item item in box.Content.Gets<Item>())
                {
                    if (item.Type == Item.Types.Food)
                    {
                        Logic.Use.Agent.Instance.Do(life, item);
                        return true;
                    }
                }
            }
        ...
        public static bool IsDeliveryObjectAdjacent(Character character)
        {
            ...
            List<Item> itemsForFoodShop = Logic.Exchange.Agent.GetItems(life, Cast.Agent.Instance.cook.IsMaterial);
        ...
        public static bool Deliver(Character character)
        {
            ...
            foreach (Character c in Exchange.Give.Availability(life))
            {
                if (c is Item item && Cast.Agent.Instance.cook.IsMaterial(item))
                {
                    Logic.Exchange.Give.Do(life, shopkeeper, item, item.Count);
                }
            }
        ...
        public static bool AutoPathToDeliveryObject(Character character)
        {
            ...
            foreach (Map map in Infrastructure.Agent.GetMaps(Map.Types.Restaurant))
            {
                if (map.Scene == life.Birthplace.Scene)
                {
                    ...
                    if (shopkeeper != null)
                    {
                        Logic.Move.Walk.FollowShortest(life, map);
                        return true;
                    }
                }
            }
        ...
        public static bool HasStorageObject(Character character)
        {
            ...
            bool hasContainer = life.Birthplace.Content.Has<Item>(i => i.Config.Id == global::Data.Constant.MiscellaneousContainer);
            bool hasMiscellaneous = Logic.Exchange.Agent.GetItemsInBag(life, IsMiscellaneous).Count > 0;
        ...
        public static bool IsStorageObjectAdjacent(Character character)
        {
            ...
            return life.Map.Content.Has<Item>(i => i.Config.Id == global::Data.Constant.MiscellaneousContainer);
        }
        ...
        public static bool Store(Character character)
        {
            ...
            if (life.Birthplace == null) return false;
            if (life.Map != life.Birthplace) return false;
            Item obj = life.Birthplace.Content.Get<Item>(i => i.Config.Id == global::Data.Constant.MiscellaneousContainer);
            if (obj == null) return false;
            List<Item> items = Logic.Exchange.Agent.GetItemsInBag(life, IsMiscellaneous);
        ...
        public static bool AutoPathToStorageObject(Character character)
        {
            ...
            Item obj = life.Birthplace.Content.Get<Item>(i => i.Config.Id == global::Data.Constant.MiscellaneousContainer);
            if (obj == null) return false;
            Logic.Move.Walk.FollowShortest(life, obj.Map);
            return true;
        }
        ...
        public static bool IsDeliveryLightGearShopObjectAdjacent(Character character)
        {
            ...
            List<Item> itemsForLightGearShop = Logic.Exchange.Agent.GetItems(life, Cast.Agent.Instance.sew.IsMaterial);
        ...
        public static bool IsDeliveryHeavyGearShopObjectAdjacent(Character character)
        {
            ...
            List<Item> itemsForHeavyGearShop = Logic.Exchange.Agent.GetItems(life, Cast.Agent.Instance.smith.IsMaterial);
