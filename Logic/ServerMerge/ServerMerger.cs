...
namespace Logic.ServerMerge
{
    public class ServerMerger
    {
        ...
        {
            var targetServer = Logic.Database.Agent.Instance.Content.Get<Logic.Database.Server>(s=>s.Id== config.TargetServerId);
            ...
            {
                var sourceServer = Logic.Database.Agent.Instance.Content.Get<Logic.Database.Server>(s => s.Id == sourceId);
        ...
        private void Step2_Backup()
        {
            ...
            {
                var server = Logic.Database.Agent.Instance.GetServerById(serverId);
        ...
        {
            var targetServer = Logic.Database.Agent.Instance.GetServerById(config.TargetServerId);
            var targetConnStr = Logic.Config.MySQL.GetConnectionString(targetServer.Id);

            foreach (var sourceId in config.SourceServerIds)
            {
                if (sourceId == config.TargetServerId) continue;

                var sourceServer = Logic.Database.Agent.Instance.GetServerById(sourceId);
                var sourceConnStr = Logic.Config.MySQL.GetConnectionString(sourceServer.Id);
        ...
        {
            foreach (var server in Logic.Database.Agent.Instance.Content.Gets<Logic.Database.Server>())
            {
