...
namespace Logic
{
    public static class SafetyNet
    {
        ...
        public static bool Execute(Func<bool> func, string context)
        {
            ...
            { 
                LogError(ex, context);
                if (global::Data.Agent.Instance.IsDevelopment) throw;
                return false; 
            }
        }
        ...
        {
            if (entity is global::Data.Life life && !(entity is global::Data.Player))
            {
                Utils.Debug.Log.Error("SAFETY_NET", $"Life[{entity.GetHashCode()}]累计{MaxEntityErrors}次错误，已移除");
                try { global::Data.Agent.Instance.Remove(life); } catch { }
            }
            else if (entity is global::Data.Player player)
            {
