...
namespace Logic.Cast
{
    internal static class Helper
    {
        ...
        {
            if (part == null) return;
            var life = part.Parent as global::Data.Life;
            if (life == null) return;
            ...
            foreach (var material in materials)
            {
                ...
                var item = life.Load<global::Data.Config.Item, Item>(materialId, dropCount);
                Exchange.Receive.Do(life.Map, item, dropCount);
            }

            life.Remove(part);
            Broadcast.Instance.Local(life, [Text.Agent.Instance.Id(global::Data.Text.Labels.PartBroken)], ("sub", life), ("part", part));
        }
        ...
        internal static void ApplyDamageToLife(Life sub, Life obj, Part part, double damagement)
        {
            ...
            else
            {
                ...
                {
                    Broadcast.Instance.Battle(obj, [Text.Agent.Instance.Id(global::Data.Text.Labels.Damage)], ("obj", obj), ("part", part), ("damagement", $"{damagement:F0}"));
                    part.Hp -= (int)damagement;
                }
            }
        }
        ...
        internal static void ApplyDamageToItem(Life sub, Item item)
        {
            ...
            if (item.Durability <= 0)
            {
                ...
                {
                    item.Count -= 1;
                    var newItem = item.Load<global::Data.Config.Item, Item>(item.Config.Id, item.Count);
        ...
        private static void ProcessDropOnAttack(Life sub, Item item)
        {
            ...
            global::Data.Map targetMap = item.Map ?? sub.Map;
            ...
            foreach (var dropTag in dropTags)
            {
                ...
                var materialConfig = global::Data.Config.Agent.Instance.Content.Get<global::Data.Config.Item>(c => c.Id == itemId);
        ...
        {
            Broadcast.Instance.Local(item, [Text.Agent.Instance.Id(global::Data.Text.Labels.Destroy)], ("sub", sub), ("item", item));
        ...
        private static void DropContainerContents(Item container)
        {
            ...
            global::Data.Map targetMap = container.Map;
            if (targetMap == null)
            {
        ...
        private static void CheckTutorialGoldMineInteraction(Player player, Item item)
        {
            ...
            global::Data.Agent.Instance.monitor.Fire(global::Data.Item.Event.Used, player, item);
        }
    }
}
