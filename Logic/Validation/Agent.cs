...
namespace Logic.Validation
{
    ...
    public class Agent : Basic.Ability
    {
        ...
        {
            var validation = Logic.Validation.Agent.Instance;

            ValidateSceneData();

            validation.monitor.Fire(Logic.Validation.Event.Completed, "Binary");
        }
        private void ValidateSceneData()
        {
            ...
            {
                validation.Create<Logic.Validation.Error>("�ļ�������", "Binary", "Scene", "Shortest.bin");
                return;
            }
            ...
            {
                validation.Create<Logic.Validation.Error>("�ļ�Ϊ��", "Binary", "Scene", "Shortest.bin");
                return;
            }

            try
            {
                var scenes = Utils.Binary.Deserialize<List<Logic.Database.Scene>>(binPath, Utils.SerializeFormat.Binary);

                if (scenes == null)
                {
                    validation.Create<Logic.Validation.Error>("�����л�ʧ��", "Binary", "Scene", "Shortest.bin");
                    return;
                }

                if (scenes.Count == 0)
                {
                    validation.Create<Logic.Validation.Error>("����Ϊ��", "Binary", "Scene", "Shortest.bin");
                    return;
                }
                ...
                foreach (var scene in scenes)
                {
                    if (scene == null)
                    {
                        validation.Create<Logic.Validation.Error>("����Ϊnull", "Binary", "Scene", "δ֪Scene");
                        continue;
                    }
                    ...
                    if (scene.maps == null)
                    {
                        validation.Create<Logic.Validation.Error>("maps����Ϊnull", "Binary", "Scene", scene.id.ToString());
                    }
                    else if (scene.maps.Count == 0)
                    {
                        validation.Create<Logic.Validation.Error>("maps����Ϊ��", "Binary", "Scene", scene.id.ToString());
                    }

                    if (scene.shortest == null)
                    {
                        validation.Create<Logic.Validation.Error>("shortest����Ϊnull", "Binary", "Scene", scene.id.ToString());
                    }
                }
            }
            catch (System.Exception ex)
            {
                validation.Create<Logic.Validation.Error>("�����л��쳣", "Binary", "Scene", ex.Message);
            }
        }
        /// <summary>
        /// ��֤Design����
        ...
        private void ValidateDesignData()
        {
            ...
            // ֪ͨ��֤��ܣ�Design��֤���
            validation.monitor.Fire(Logic.Validation.Event.Completed, "Design");
        }
        ...
        {
            var validation = Logic.Validation.Agent.Instance;
            var items = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Item>();
            ...
            foreach (var item in items)
            {
                ...
                {
                    var validValues = System.Enum.GetNames(typeof(Logic.Item.Types));
                    if (!validValues.Contains(item.type))
                    {
                        validation.Create<Logic.Validation.Error>("Invalid enum value", "Design", "Item", item.cid);
                    }
                }
            }
        }
        ...
        {
            var validation = Logic.Validation.Agent.Instance;
            var lifes = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Life>();
            ...
            foreach (var life in lifes)
            {
                ...
                {
                    validation.Create<Logic.Validation.Error>("Parts field is required", "Design", "Life", life.cid, 
                ...
                if (!parts.Any())
                {
                    validation.Create<Logic.Validation.Error>("Parts field is empty", "Design", "Life", life.cid,
                ...
                foreach (var part in parts)
                {
                    ...
                    {
                        validation.Create<Logic.Validation.Error>("Invalid part name", "Design", "Life", life.cid,
                ...
                {
                    validation.Create<Logic.Validation.Error>("Missing required part", "Design", "Life", life.cid,
        ...
        {
            var validation = Logic.Validation.Agent.Instance;
            var scenes = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Scene>();
            ...
            foreach (var scene in scenes)
            {
                ...
                {
                    validation.Create<Logic.Validation.Error>("Multilingual key not found", "Design", "Scene", scene.cid, 
        ...
        private void ValidateMaps()
        {
            ...
            {
                validation.Create<Logic.Validation.Error>("File not found", "Design", "Map", "地图坐标.csv");
                return;
            }

            var cells = Utils.Csv.LoadAsCells(worldPath);
            var maps = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Map>();
            var scenes = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Scene>();
            var lives = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Life>();
            var items = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Item>();
            ...
            for (int y = 0; y < cells.Count; y++)
            {
                for (int x = 0; x < cells[y].Count; x++)
                {
                    ...
                    if (!string.IsNullOrEmpty(cellValue))
                    {
                        ...
                        {
                            validation.Create<Logic.Validation.Error>("Map CID not found in config", "Design", "Map", cellValue, 
                        ...
                        else
                        {
                            ...
                            {
                                validation.Create<Logic.Validation.Error>("Scene CID not found in config", "Design", "Scene", sceneCid,
            ...
            foreach (var map in maps)
            {
                ...
                {
                    validation.Create<Logic.Validation.Error>("Unused map config", "Design", "Map", map.cid);
                }

                if (string.IsNullOrEmpty(map.name))
                {
                    validation.Create<Logic.Validation.Error>("Map name is empty", "Design", "Map", map.cid,
                        $"Map [{map.cid}] has empty or null name field. Please set a valid name in 地图.csv");
                }

                if (string.IsNullOrEmpty(map.type))
                {
                    validation.Create<Logic.Validation.Error>("Map type is empty", "Design", "Map", map.cid,
                ...
                {
                    validation.Create<Logic.Validation.Error>("Multilingual key not found", "Design", "Map", map.cid, 
                ...
                if (map.characters != null)
                {
                    foreach (string characterEntry in map.characters)
                    {
                        ...
                        foreach (var (cid, hasLevel) in processedCids)
                        {
                            ...
                            {
                                validation.Create<Logic.Validation.Error>("Missing level range", "Design", "Life", cid);
                            }
                            else if (!lifeCids.Contains(cid))
                            {
                                validation.Create<Logic.Validation.Error>("Character CID not found", "Design", "Character", cid);
                            }
                        }
                    }
                }
        ...
        private void ValidateIntegrity()
        {
            ...
            if (!items.Any())
            {
                validation.Create<Logic.Validation.Error>("Data is empty or load failed", "Design", "Item", "All Item Data");
            }

            if (!maps.Any())
            {
                validation.Create<Logic.Validation.Error>("Data is empty or load failed", "Design", "Map", "All Map Data");
            }
        }
