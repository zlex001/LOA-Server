    ...
    public class Daily
    {
        ...
        public void OnPeriodChanged(Agent.Period oldPeriod)
        {
            ...
            if (oldPeriod == Agent.Period.Night && currentPeriod == Agent.Period.Dawn)
            {
                ...
                {
                    Logic.Broadcast.Instance.System(player, new object[] { Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.Sunrise) });
                }
            }
            else if (oldPeriod == Agent.Period.Evening && currentPeriod == Agent.Period.Night)
            {
                foreach (Player player in global::Data.Agent.Instance.Content.Gets<Player>())
                {
                    Logic.Broadcast.Instance.System(player, new object[] { Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.Sunset) });
                }
            }
        ...
        {
            foreach (var scene in global::Data.Agent.Instance.Content.Gets<global::Data.Scene>())
            {
                foreach (var map in scene.Content.Gets<global::Data.Map>())
                {
                    foreach (var life in map.Content.Gets<global::Data.Life>())
                    {
                        if (life.Injury > 0)
                        {
                            life.Injury -= 1;
        ...
        private void UpdateAge()
        {
            ...
            foreach (var scene in global::Data.Agent.Instance.Content.Gets<global::Data.Scene>())
            {
                foreach (var map in scene.Content.Gets<global::Data.Map>())
                {
                    foreach (var life in map.Content.Gets<global::Data.Life>().Where(l => !(l is Player)))
                    {
                        life.Age += 1.0;
                    }
                }
        ...
        private void NPC()
        {
            foreach (var scene in global::Data.Agent.Instance.Content.Gets<global::Data.Scene>(s => s is not Maze))
            {
                scene.UpdateLife();
            }
        }
        ...
        {
            var scenes = global::Data.Agent.Instance.Content.Gets<global::Data.Scene>().ToList();
                ...
                foreach (var map in scene.Content.Gets<Map>())
                {
                    foreach (var item in map.Content.Gets<Item>())
                    {
                        if (item?.Config?.Tags != null && Utils.Tag.HasPrefix(item.Config.Tags, "Generate"))
                        {
                            ...
                            {
                                var materialConfigs = new List<global::Data.Config.Item>(8);
                                ...
                                for (int i = 0; i < parts.Length; i++)
                                {
                                    ...
                                    {
                                        var cfg = global::Data.Config.Agent.Instance.Content.Get<global::Data.Config.Item>(x => x.Id == id);
                                        if (cfg != null)
                                        {
                                ...
                                if (materialConfigs.Count > 0)
                                {
                                    ...
                                    if (currentTotalValue < maxCapacity)
                                    {
                                        ...
                                        if (totalWeight > 0f)
                                        {
                                            ...
                                            float acc = 0f;
                                            global::Data.Config.Item selected = null;
