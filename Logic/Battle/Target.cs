...
namespace Logic.Battle
{
    public static class Target
    {
        public static List<Character> Get(Life life)
        {
            ...
            if (life == null || life.State.Is(global::Data.Life.States.Unconscious))
            {
                return targets;
            }
            if (life.Map == null)
            ...
            foreach (var kvp in life.Relation.Where(kvp => kvp.Value < 0))
            {
                ...
                {
                    if (!target.State.Is(global::Data.Life.States.Unconscious) && target.Map != null && target.Map == life.Map)
                    {
                        targets.Add(target);
                    }
                    else
