...
namespace Logic.Mall
{
    public static class Purchase
    {
        ...
        public static bool Can(Player player, global::Data.Config.Mall mallConfig, int count, out FailReason reason)
        {
            ...
            bool isVirtualProduct = mallConfig.Type == global::Data.Config.Mall.Types.Subscription ||
                                    mallConfig.Type == global::Data.Config.Mall.Types.Experience;
            if (!isVirtualProduct && !CanReceive(player)) { reason = FailReason.CannotReceive; return false; }

            return true;
        }
        
        public static bool Can(Player player, global::Data.Config.Mall mallConfig, int count)
        {
            return Can(player, mallConfig, count, out _);
        }

        private static bool NeedPackage(global::Data.Config.Mall mallConfig)
        {
            return mallConfig.Items != null && mallConfig.Items.Count > 1;
        }

        private static Item CreatePackage(Player player, global::Data.Config.Mall mallConfig, int count)
        {
            var bag = player.Load<global::Data.Config.Item, Item>(PackageBagId, 1);
            foreach (var item in mallConfig.Items)
            {
                bag.Load<global::Data.Config.Item, Item>(item.Key, item.Value * count);
            }
            return bag;
        }

        private static void DeliverDirect(Player player, global::Data.Config.Mall mallConfig, int count)
        {
            foreach (var item in mallConfig.Items)
            {
                var created = player.Load<global::Data.Config.Item, Item>(item.Key, item.Value * count);
                Exchange.Receive.Do(player, created, created.Count);
            }
        }

        public static bool Do(Player player, global::Data.Config.Mall mallConfig, int count, out FailReason reason)
        {
            ...
            {
                case global::Data.Config.Mall.Types.Subscription:
                    DeliverSubscription(player, mallConfig, count);
                    break;
                case global::Data.Config.Mall.Types.Experience:
        ...
        private static void DeliverSubscription(Player player, global::Data.Config.Mall mallConfig, int count)
        {
            int targetCardValue = mallConfig.Value;
            
            // Each purchase adds 30 days per count
            int daysToAdd = global::Data.Constant.MonthlyCardDurationDays * count;
        ...
        private static void DeliverExperience(Player player, global::Data.Config.Mall mallConfig, int count)
        {
        ...
        public static bool Do(Player player, global::Data.Config.Mall mallConfig, int count)
        {
            return Do(player, mallConfig, count, out _);
        }

        private static void BroadcastPurchase(Player player, global::Data.Config.Mall mallConfig, int count, int totalPrice)
        {
            string mallName = Text.Agent.Instance.Get(mallConfig.Name, player);
            Logic.Broadcast.Instance.System(player,
                [Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.MallPurchase)],
                ("item", mallName),
