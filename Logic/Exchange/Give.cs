...
namespace Logic.Exchange
{
    public class Give
    {
        ...
        public static void Do(Life sub, Life obj, Life target)
        {
            ...
            {
                Broadcast.Instance.Local(obj, [Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.GiveLifeToLife)], ("sub", sub), ("item", target), ("obj", obj));
        ...
        public static void Do(Life sub, Life obj, Item target, int count)
        {
            if (Can(sub, obj, target, count))
            {
                ...
                if (sub is Player player && obj.Category == Life.Categories.Animal)
                {
                    ...
                    if (Logic.Pet.Capture.TryCapture(player, obj, target))
                    {
                        return;
                    }
                }
                ...
                // 普通给予流程
                Broadcast.Instance.Local(obj, [Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.GiveItemToLife)], ("sub", sub), ("target", target), ("obj", obj), ("count", count.ToString()));
        ...
        public static void Do(Life sub, Item obj, Life target)
        {
            if (Can(sub, obj, target))
            {
                Broadcast.Instance.Local(obj, [Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.GiveLifeToItem)], ("sub", sub), ("item", target), ("obj", obj));
                Receive.Do(obj, target);
            }
        }
        public static void Do(Life sub, Item obj, Item target, int count)
        {
            if (Can(sub, obj, target, count))
            {
                Broadcast.Instance.Local(obj, [Logic.Text.Agent.Instance.Id(global::Data.Text.Labels.GiveItemToItem)], ("sub", sub), ("target", target), ("obj", obj), ("count", count.ToString()));
                Receive.Do(obj, target, count);
                
                // Fire global event for item given (tutorial, quests, etc.)
                if (sub is Player player)
                {
                    global::Data.Agent.Instance.monitor.Fire(global::Data.Character.Event.Given, player, target, obj);
                }
            }
        }
        ...
        public static bool ShouldShowItem(Option option, Item item, string operationType)
        {
            ...
            return string.IsNullOrEmpty(filter) || Logic.Text.Name.Item(item, option.Player).ToLower().Contains(filter);
        }
