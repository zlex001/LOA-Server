...
namespace Basic
{
    ...
    {
        public O Load<C, O>(int id) where O : Element where C : Logic.Config.Ability
        {
            C config = Logic.Config.Agent.Instance.GetCached<C>(id);
            if (config == null)
            {
                config = Logic.Config.Agent.Instance.Content.Get<C>(c => c.Id == id);
            }
        ...
        public O Load<C, O>(int id, params object[] args) where O : Element where C : Logic.Config.Ability
        {
            C config = Logic.Config.Agent.Instance.GetCached<C>(id);
            if (config == null)
            {
