...
namespace Logic.Database
{
    public partial class Agent : Basic.MySql
    {
        ...
        public override void Init(params object[] args)
        {
            ...
            {
                var serverId = Logic.Agent.Instance.ServerId;
        ...
        {
            Save<Device>(Logic.Config.MySQL.ConnectionString);
            Save<Player>(Logic.Config.MySQL.ConnectionString);
        }
        public void Delete(Basic.Data data)
        {
            Delete(Logic.Config.MySQL.ConnectionString, data);
        }
        ...
        private void ValidateDatabaseData()
        {
            ...
            // 通知验证框架：Database验证完成
            validation.monitor.Fire(Logic.Validation.Event.Completed, "Database");
        }
        ...
        {
            var validation = Logic.Validation.Agent.Instance;
            var players = Content.Gets<Logic.Database.Player>();
            var validSkillCids = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Skill>()
                .Where(config => !string.IsNullOrEmpty(config.cid))
                .Select(config => config.cid)
                .ToHashSet();
            var validItemCids = Logic.Design.Agent.Instance.Content.Gets<Logic.Design.Item>()
                .Where(config => !string.IsNullOrEmpty(config.cid))
                .Select(config => config.cid)
                .ToHashSet();
            ...
            foreach (var player in players)
            {
                ...
                {
                    validation.Create<Logic.Validation.Error>("ID为空", "Database", "Player", player.Id?.ToString() ?? "null");
                }
            }

            // 验证技能CID和道具CID
            var playerRawData = Query(Logic.Config.MySQL.ConnectionString, "Player");
            ...
            foreach (var (cid, playerId, field) in usedSkillCids)
            {
                if (!validSkillCids.Contains(cid) && !IsAutoHandleCid(cid))
                {
                    ...
                    validation.Create<Logic.Validation.Error>("技能CID不存在", "Database", "Skill", cid, details);
                }
            }
            ...
            foreach (var (cid, playerId, field) in usedItemCids)
            {
                ...
                {
                    var details = $"玩家[{playerId}]的{field}字段";
                    validation.Create<Logic.Validation.Error>("道具CID不存在", "Database", "Item", cid, details);
                }
            }
        }
