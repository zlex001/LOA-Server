...
namespace Logic.Database
{
    ...
    public class Item : Basic.Data
    {
        ...
        public Logic.Part.Types Part { get; set; } = Logic.Part.Types.Hand; // 记录Item所在的Part类型，默认Hand
        ...
        public override void Init(params object[] args)
        {
            ...
            {
                if (partObj is string partStr && System.Enum.TryParse<Logic.Part.Types>(partStr, out var partType))
                {
                    Part = partType;
                }
                else if (partObj is int partInt)
                {
                    Part = (Logic.Part.Types)partInt;
                }
            }
            else
            {
                Part = Logic.Part.Types.Hand; // 旧数据默认Hand
            }
