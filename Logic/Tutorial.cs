...
namespace Logic
{
    ...
    public class Tutorial
    {
        ...
        public void Init()
        {
            ...
            // Register event listeners on global::Data.Agent singleton (global scope)
            global::Data.Agent.Instance.monitor.Register(global::Data.Player.Event.StoryComplete, OnStoryComplete);
            global::Data.Agent.Instance.monitor.Register(global::Data.Item.Event.Used, OnItemUsed);
            global::Data.Agent.Instance.monitor.Register(global::Data.Item.Event.Picked, OnItemPicked);
            global::Data.Agent.Instance.monitor.Register(global::Data.Character.Event.Given, OnCharacterGiven);
            global::Data.Agent.Instance.monitor.Register(global::Data.Life.Event.Die, OnLifeDie);
            global::Data.Agent.Instance.monitor.Register(global::Data.Character.Event.GoTo, OnCharacterGoTo);
        ...
        private void CacheConfigIds()
        {
            ...
            var tutorialShore = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Map>(m => m.cid == "遗迹-岸边");
            _tutorialShoreMapId = tutorialShore?.id ?? 0;

            var tutorialSand = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Map>(m => m.cid == "遗迹-沙地");
            _tutorialSandMapId = tutorialSand?.id ?? 0;

            var tutorialTower = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Map>(m => m.cid == "遗迹-通天塔");
            _tutorialTowerMapId = tutorialTower?.id ?? 0;

            var goldOre = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Item>(i => i.cid == "金矿石");
            _goldOreItemId = goldOre?.id ?? 0;

            var rawMeat = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Item>(i => i.cid == "生肉");
            _rawMeatItemId = rawMeat?.id ?? 0;

            var goldMine = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Item>(i => i.cid == "金矿");
            _goldMineItemId = goldMine?.id ?? 0;

            var lizard = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Life>(l => l.cid == "蜥蜴");
            _lizardLifeId = lizard?.id ?? 0;

            var stele = global::Data.Design.Agent.Instance.Content.Get<global::Data.Design.Item>(i => i.cid == "石碑");
        ...
        /// </summary>
        private global::Data.Copy CreateTutorialCopy(Player player)
        {
            ...
            var tutorialMap = global::Data.Agent.Instance.Content.Get<global::Data.Map>(m =>
            ...
            var copyConfig = new global::Data.Config.Quest.Copy
            {
                scope = 5,
                characters = new Dictionary<int, List<global::Data.Config.Quest.Character>>()
            };

            // Add characters for sand map (gold mine and lizard)
            if (_tutorialSandMapId > 0)
            {
                var sandCharacters = new List<global::Data.Config.Quest.Character>();
                if (_goldMineItemId > 0)
                {
                    sandCharacters.Add(new global::Data.Config.Quest.Character { id = _goldMineItemId, count = 1 });
                }
                if (_lizardLifeId > 0)
                {
                    sandCharacters.Add(new global::Data.Config.Quest.Character { id = _lizardLifeId, count = 1, min = 1, max = 1 });
                }
            ...
            {
                copyConfig.characters[_tutorialTowerMapId] = new List<global::Data.Config.Quest.Character>
                {
                    new global::Data.Config.Quest.Character { id = _steleItemId, count = 1 }
                };
            }

            // Create the copy
            var copy = global::Data.Agent.Instance.Create<global::Data.Copy>(tutorialMap, copyConfig);
            if (copy != null)
            {
        ...
        /// </summary>
        private void PlacePlayerInCopy(Player player, global::Data.Copy copy)
        {
            ...
            var targetMap = copy.Content.Get<global::Data.Map>(m =>
        ...
        public void Complete(Player player)
        {
            ...
            var destination = global::Data.SpawnPoint.GetRandomInitialMap();
            if (destination != null)
            {
        ...
        private void SendExploreHint(Player player, PlayerState state)
        {
            ...
            string hintText = Logic.Text.Agent.Instance.GetByCid(hintCid, player);
        ...
        {
            string hintText = Logic.Text.Agent.Instance.GetByCid("tutorial_pickup", player);
        ...
        private int[] GetMapPos(int mapId, Player player)
        {
            ...
            {
                var targetMap = currentMap.Copy.Content.Get<global::Data.Map>(m => m.Config.Id == mapId);
                if (targetMap != null)
                {
            ...
            {
                var targetMap = currentMap.Scene.Content.Get<global::Data.Map>(m => m.Config.Id == mapId);
                if (targetMap != null)
                {
        ...
        {
            var textAgent = Logic.Text.Agent.Instance;
