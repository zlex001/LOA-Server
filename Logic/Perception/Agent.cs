...
namespace Logic.Perception
{
    public class Agent : Agent<Agent>
    {
        ...
        public void Init()
        {
            ...
            global::Data.Player.GetVisibleCharacters = GetVisibleCharacters;
            
            // Listen for Map add/remove to register content listeners
            global::Data.Agent.Instance.Content.Add.Register(typeof(Map), OnMapAdded);
            global::Data.Agent.Instance.Content.Remove.Register(typeof(Map), OnMapRemoved);

            // Initialize for existing maps and their characters
            foreach (var map in global::Data.Agent.Instance.Content.Gets<Map>())
            {
                RegisterMap(map);
            }
        }
