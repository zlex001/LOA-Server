    ...
    public static class Walk
    {
        ...
        public static void Do(Life life, Map destination)
        {
            if (Can(life, destination))
            {
                ...
                var directionText = Logic.Text.Agent.Instance.Id(directionLabel);
                //Broadcast.Instance.Local(life, [global::Data.Text.Instance.FirstIdOf(global::Data.Text.Labels.Leave)], ("sub", life), ("direction", directionText));
        ...
        private static void FollowShortestCrossScene(Life life, Map destination)
        {
            ...
            if (!global::Data.Agent.Instance.Content.Has<Scene>(s => s.Database.id == paths[0], out var nextScene))
            {
                return;
            }
        ...
        public static List<int[]> Area(global::Data.Player player)
        {
            var result = new List<int[]>();
            if (player.Map?.Scene != null)
            {
                foreach (var m in player.Map.Scene.Content.Gets<global::Data.Map>())
                {
