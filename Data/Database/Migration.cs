...
namespace Data.Database
{
    public static class Migration
    {
        ...
        {
            var players = Database.Agent.Instance.Query(global::Data.Config.MySQL.ConnectionString, "Player");
            int migratedCount = 0;
        ...
        {
            var merchandises = Database.Agent.Instance.Query(global::Data.Config.MySQL.ConnectionString, "Merchandise");
            var players = Database.Agent.Instance.Query(global::Data.Config.MySQL.ConnectionString, "Player");
            ...
            foreach (var player in players)
            {
                ...
                if (merchandiseBySupplier.TryGetValue(playerId, out var playerMerchandises))
                {
                    ...
                    if (merchandiseItems.Count > 0)
                    {
                        ...
                        Database.Agent.Instance.ExecuteNonQuery(global::Data.Config.MySQL.ConnectionString,
        ...
        private static bool IsSkillCid(string cid)
        {
            try
            {
                ...
                var skillConfigs = global::Data.Design.Agent.Instance.Content.Gets<global::Data.Design.Skill>();
        ...
        private static void SavePlayer(Dictionary<string, object> player)
        {
            ...
            Database.Agent.Instance.ExecuteNonQuery(global::Data.Config.MySQL.ConnectionString, sql, parameters);
        }
        ...
        private static void DropItemsColumn()
        {
            try
            {
                ...
                Database.Agent.Instance.ExecuteNonQuery(global::Data.Config.MySQL.ConnectionString, dropColumnSql, new Dictionary<string, object>());
