...
namespace Data.Design
{
    public class Quest : Ability
    {
        ...
        private static string ConvertTrigger(string chineseTrigger)
        {
            ...
            {
                "抵达" => "global::Data.Map+Event.Arrived",
                "交谈" => "global::Data.Life+Event.Talked",
                "拾取" => "global::Data.Item+Event.Picked",
                "给予" => "global::Data.Character+Event.Given",
                "死亡" => "global::Data.Life+Event.Die",
                "进入" => "global::Data.Item+Event.Enter",
        ...
        private static int GetIdOrRecord<T>(string cid, Func<T, bool> predicate, string typeName) where T : class
        {
            ...
            if (item == null)
            {
                global::Data.Validation.Agent.Instance.Create<global::Data.Validation.Error>("Reference not found", "Design", "Quest", cid,
        ...
        private static void ConvertConditionTreeReferences(global::Data.Config.ConditionNode node)
        {
            if (node == null)
                return;

            if (node is global::Data.Config.SingleCondition single)
            {
                single.Value = ConvertCondition(single.Value);
            }
            else if (node is global::Data.Config.AndCondition and)
            {
                ConvertConditionTreeReferences(and.Left);
                ConvertConditionTreeReferences(and.Right);
            }
            else if (node is global::Data.Config.OrCondition or)
            {
                ConvertConditionTreeReferences(or.Left);
                ConvertConditionTreeReferences(or.Right);
            }
            else if (node is global::Data.Config.NotCondition not)
            {
