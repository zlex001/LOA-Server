...
namespace Data.Design
{
    public class BehaviorTree : Ability
    {
        ...
        public static void Convert()
        {
            ...
            foreach (BehaviorTree behaviorTree in global::Data.Design.Agent.Instance.Content.Gets<BehaviorTree>())
            {
                int[] nodeIds = string.IsNullOrEmpty(behaviorTree.nodes)
                    ? Array.Empty<int>()
                    : behaviorTree.nodes.Split(',')
                        .Select(cid => global::Data.Design.Agent.Instance.Content.Get<BehaviorTree>(bt => bt.cid == cid.Trim()))
                        .Where(bt => bt != null)
                        .Select(bt => bt.id)
                        .ToArray();
