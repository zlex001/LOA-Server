...
namespace Domain.State
{
    ...
    {
        public static readonly double LpDrainPerSecond = 100.0 / Logic.Constant.NormalLpDepletionSeconds;
        ...
        public static void RestoreHp(Logic.Life life, double hpPerSecond)
        {
            if (life.Lp <= 0) return;
            if (!life.Content.Has<Logic.Part>(p => p.Hp < p.MaxHp)) return;

            var tickInterval = life.State.CurrentKey == Logic.Life.States.Battle 
                ? Logic.Constant.StateBattleTickIntervalMs 
                : (life.State.CurrentKey == Logic.Life.States.Normal 
                    ? Logic.Constant.StateNormalTickIntervalMs 
                    : Logic.Constant.StateDefaultTickIntervalMs);

            int healAmount = Math.Max(1, (int)(hpPerSecond * tickInterval / 1000.0));
            
            var part = life.Content.RandomGet<Logic.Part>(p => p.Hp < p.MaxHp);
            if (part != null)
            {
                part.Hp += healAmount;
            }
        ...
        public static void DrainLp(Logic.Life life, double lpPerSecond)
        {
            var tickInterval = life.State.CurrentKey == Logic.Life.States.Battle 
                ? Logic.Constant.StateBattleTickIntervalMs 
                : (life.State.CurrentKey == Logic.Life.States.Normal 
        ...
        private static void DamageFromHunger(Logic.Life life)
        {
            int damageValue = Math.Max(1, (int)(100 - life.Con / 10.0));
            var availableParts = life.Content.Gets<Logic.Part>().Where(p => p.Hp > -p.MaxHp).ToList();
        ...
        private static double CalculateBehaviorTreeInterval(Logic.Life life, bool isInitial = false)
        {
        ...
        public static void StartBehaviorTree(Logic.Life life)
        {
        ...
        public static void StopBehaviorTree(Logic.Life life)
        {
            if (life.BtTaskId > 0)
            {
                Domain.Time.Agent.Instance.Scheduler.CancelTask(life.BtTaskId);
                life.BtTaskId = 0;
            }
        }

        private static void ScheduleNextBehaviorTreeExecution(Logic.Life life, bool isInitial = false)
        {
            if (life.BtRoot == null) return;
            
            double interval = CalculateBehaviorTreeInterval(life, isInitial);
            life.BtTaskId = Domain.Time.Agent.Instance.Scheduler.Once((long)interval, (_) => 
            {
                ExecuteBehaviorTree(life);
            });
        }

        private static void ExecuteBehaviorTree(Logic.Life life)
        {
            life.BtTaskId = 0;
            ...
            if (!life.State.Is(Logic.Life.States.Normal))
            {
            ...
            if (lifeBucket != _currentBtBucket)
            {
                ...
                life.BtTaskId = Domain.Time.Agent.Instance.Scheduler.Once(50, (_) => 
                {
                    ExecuteBehaviorTree(life);
                });
                return;
        ...
        private static void OnAddLife(params object[] args)
        {
            ...
            life.State = new State<Logic.Life.States>();
