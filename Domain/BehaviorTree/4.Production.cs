...
namespace Domain.BehaviorTree
{
    ...
    public class Production
    {
        ...
        public static Life Prey(Life life, Func<Life, bool> predicate = null)
        {
            ...
            return Logic.Agent.Instance.Content.Gets<Life>()
                .Where(l => l != life)
        ...
        private static bool IsHuntableCategory(Logic.Life.Categories category)
        {
            return category == Logic.Life.Categories.Demon || 
                   category == Logic.Life.Categories.Animal;
        }

        public static Life FindBestPreyForMaster(Life companion)
        {
            if (companion == null) return null;
            if (companion.Leader is not Logic.Player player) return null;
            if (companion.Map?.Scene == null) return null;

            var candidates = Logic.Agent.Instance.Content.Gets<Life>()
                .Where(l => l != companion)
                .Where(l => l != player)
                .Where(l => l.Category == Logic.Life.Categories.Animal)
                .Where(l => !l.State.Is(Logic.Life.States.Unconscious))
        ...
        public static bool HasDismemberTarget(Character character)
        {
            ...
            Life prey = Prey(life, l => l.State.Is(Logic.Life.States.Unconscious) && life.WorkingScenes.Contains(l.Map?.Scene));
            return prey != null;
        }
        ...
        public static bool IsDismemberTargetAdjacent(Character character)
        {
            ...
            if (life.Map.Scene == null)
                return false;
            Life prey = Prey(life, l => l.State.Is(Logic.Life.States.Unconscious) && l.Map == life.Map);
            if (prey == null)
                return false;
            return true;
        }
        ...
        public static bool AutoPathToCollectionTarget(Character character)
        {
            ...
            if (nearestMap != null)
            {
                Domain.Move.Walk.FollowShortest(life, nearestMap);
            }
            return true;
        }
        ...
        public static bool DismemberTarget(Character character)
        {
            ...
            Life prey = Prey(life, l => l.State.Is(Logic.Life.States.Unconscious));
            if (prey == null) return false;
            Domain.Battle.Round.DoAttack(life, prey);
            return true;
        }
        ...
        public static bool AutoPathToDismemberTarget(Character character)
        {
            ...
            Life prey = Prey(life, l => l.State.Is(Logic.Life.States.Unconscious) && life.WorkingScenes.Contains(l.Map?.Scene));
            if (prey == null) return false;
            Domain.Move.Walk.FollowShortest(life, prey.Map);
            return true;
        }
        ...
        public static bool HuntTarget(Character character)
        {
            ...
            if (prey == null)
                return false;
            Domain.Battle.Agent.Instance.Hostile(life, prey);
            return true;
        }
        ...
        {
            var companion = character as Logic.Life;
            if (companion == null) return false;
            if (companion.Leader is not Logic.Player) return false;
        ...
        public static bool LeadMasterToHunt(Character character)
        {
            var companion = character as Logic.Life;
            if (companion == null) return false;
            if (companion.Leader is not Logic.Player player) return false;

            var target = FindBestPreyForMaster(companion);
            if (target == null) return false;

            Broadcast.Instance.Local(companion, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CompanionLeadHunt)], ("sub", companion), ("target", target));

            Domain.Move.Walk.FollowShortest(companion, target.Map);

            Domain.Time.Agent.Instance.Scheduler.Once(300, (_) =>
            {
                if (player != null && !player.State.Is(Logic.Life.States.Unconscious))
                {
                    Domain.Move.Walk.FollowShortest(player, target.Map);
                    Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CompanionLeadHuntFollow)], ("sub", player), ("target", companion));
                }
            });
        ...
        {
            if (character == null) return false;
            return Logic.Agent.Instance.Content.Has<Map>(m => m.Content.Has<Item>(IsOre));
        }
        ...
        {
            if (character == null) return false;
            return Logic.Agent.Instance.Content.Has<Map>(m => m.Content.Has<Item>(IsOreStone));
        }
        ...
        private static Movement SelectMiningMovement(Logic.Life life)
        {
            if (life == null) return null;
