...
namespace Domain
{
    public class Analytics
    {
        ...
        private int ActivePlayerCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.Active(dateTime));

        #region User Growth Metrics (增长)
        private int NewDeviceCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Device>(d => d.New(dateTime));
        private int NewDevicePlayerCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Device>(d => d.New(dateTime) && !string.IsNullOrEmpty(d.player));
        private int NewValidDevicePlayerCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Device>(d => 
        {
            if (!d.New(dateTime) || string.IsNullOrEmpty(d.player)) 
                return false;
            
            var player = Logic.Database.Agent.Instance.Content.Get<Logic.Database.Player>(p => p.Id == d.player);
            return player != null && player.skills.Count > 0;
        });
        private int NewPlayerCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime));
        private int NewValidPlayerCount(DateTime dateTime) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime) && p.skills.Count > 0);
        #endregion

        #region Retention Metrics (留存)
        private int RetainedUsers(DateTime dateTime, int days) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime.AddDays(-days)) && p.Active(dateTime));
        private int LostUsers(DateTime dateTime, int days) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime.AddDays(-days)) && !p.Active(dateTime));
        private int WinBackUsers(DateTime dateTime, int days) => Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime.AddDays(-(days + 1))) && !p.Active(dateTime.AddDays(-1)) && p.Active(dateTime));
        ...
        private double AverageUserLifetime => (double)Logic.Database.Agent.Instance.Content.Gets<Logic.Database.Player>().Select(p => p.activitys.Count).Sum() / Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>();
        #endregion

        #region Monetization Metrics (付费指标)
        private double ConversionRate(DateTime dateTime)
        {
            int @new = Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime));
            int retained = Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.New(dateTime) && p.NewPaying(dateTime));
        ...
        {
            double revenue = 0;
            foreach (Logic.Database.Player player in Logic.Database.Agent.Instance.Content.Gets<Logic.Database.Player>(p => p.Active(dateTime)))
            {
                foreach (Logic.Database.Payment payment in player.payments)
                {
        ...
        private double ARPU(DateTime dateTime)
        {
            ...
            int activeUsers = Logic.Database.Agent.Instance.Content.Count<Logic.Database.Player>(p => p.Active(dateTime));
        ...
        {
            double revenue = Revenue(dateTime);
            var activeUsers = Logic.Database.Agent.Instance.Content.Gets<Logic.Database.Player>(p => p.Active(dateTime));
        ...
        public void SaveToDatabase(DateTime dateTime)
        {
            try
            {
                ...
                var dbAnalytics = new Logic.Database.DailyAnalytics
                {
                    Date = dateTime,
                ...
                Logic.Database.Agent.Instance.SaveDailyAnalytics(dbAnalytics);
            }
            catch (Exception ex)
            {
        ...
        public List<Daily> QueryFromDatabase(DateTime startDate, DateTime endDate)
        {
            try
            {
                var dbResults = Logic.Database.Agent.Instance.QueryDailyAnalytics(startDate, endDate);
