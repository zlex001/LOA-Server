    ...
    public class Hot
    {
        ...
        private async void OnVersion(params object[] args)
        {
            ...
            var response = new
            {
                ...
                texts = new
                {
                    checking = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.CheckingHotVersion, language),
                    check_failed = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.VersionCheckFailed, language)
                }
            };
        ...
        private async void OnCheck(params object[] args)
        {
            ...
            var response = new
            {
                ...
                texts = new
                {
                    comparing = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.ComparingResources, language),
                    resources_updated = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.NoUpdateNeeded, language),
                    start_downloading = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.StartDownloading, language),
                    download_complete = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.UpdateComplete, language),
                    download_failed = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.DownloadFailed, language),
                    loading_metadata = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.LoadingMetadata, language),
                    launching_game = Domain.Text.Agent.Instance.Get(Logic.Text.Labels.LaunchingGame, language)
                }
            };
