...
namespace Domain
{
    public class EconomyMonitor
    {
        ...
        private void GenerateReport()
        {
            ...
            foreach (var scene in Logic.Agent.Instance.Content.Gets<Scene>())
            {
        ...
        private string GetSceneName(Scene scene)
        {
            ...
            if (Logic.Text.Instance.Multilingual.TryGetValue(scene.Config.Name, out var map) 
                && map.TryGetValue(Logic.Text.Languages.ChineseSimplified, out var name))
            {
                return name;
            }
