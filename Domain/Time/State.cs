...
namespace Domain.Time
{
    public class State
    {
        ...
        public void Init()
        {
            Logic.Agent.Instance.Content.Add.Register(typeof(Life), OnLifeAdded);
            Logic.Agent.Instance.Content.Remove.Register(typeof(Life), OnLifeRemoved);
            
            foreach (var life in Logic.Agent.Instance.Content.Gets<Life>())
            {
                AddToStateList(life);
                life.State.monitor.Register(Basic.State<Life.States>.Event.Changed, (args) => OnLifeStateChanged(life, args));
            }

            Agent.Instance.Scheduler.Once(0, (_) => Agent.Instance.Scheduler.Repeat(Logic.Constant.StateBattleTickIntervalMs, (_) => OnBattleTick()));
            ...
            Agent.Instance.Scheduler.Once(150, (_) => Agent.Instance.Scheduler.Repeat(Logic.Constant.StateDefaultTickIntervalMs, (_) => OnDefaultTick()));
