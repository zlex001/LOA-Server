...
namespace Domain.Exchange
{
    public static class Sell
    {
        ...
        public static void Do(Life sub, Life obj, Item item, int count)
        {
            ...
            {
                Broadcast.Instance.Local(obj, [Text.Agent.Instance.Id(Logic.Text.Labels.Sell)], ("sub", sub), ("item", item), ("obj", obj), ("count", count.ToString()));
                Infrastructure.Agent.Guild.Pay(obj, sub, Utils.Mathematics.AsInt(ComputePrice(item, count)));
                Receive.Do(obj, item, count);
            }
            else
            {
                Broadcast.Instance.Local(obj,[Domain.Text.Agent.Instance.Id(Logic.Text.Labels.ReceiveFail)],("sub", obj),("item", item),("count", count.ToString()));
            }
        }
    }
}
