...
namespace Domain.Exchange
{
    public class Buy
    {
        ...
        private static bool Afford(Life sub, int price, out Item money)
        {
            ...
            {
                if (character is Item item && item.Config.Id == Logic.Constant.Money)
                {
                    money = item;
                    break;
                }
        ...
        public static void Do(Life sub, Life obj, Item target, int count)
        {
            ...
            {
                Broadcast.Instance.Local(obj, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.Buy, random: true)], ("sub", sub), ("item", target), ("count", count.ToString()));
