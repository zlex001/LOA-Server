...
namespace Domain.Display
{
    ...
    {
        public static void OptionButton(Logic.Player player, int side, int index)
        {
            if (player?.Option != null)
            {
                ...
                if (target != null)
                {
                    ...
                    {
                        0 => Enum.Parse<Logic.Option.LeftPanel>(player.Option.Type.ToString()),
                        1 => Enum.Parse<Logic.Option.RightPanel>(player.Option.Type.ToString()),
        ...
        public static void OptionConfirm(Logic.Player player, int side, int index)
        {
        ...
        {
            var leftItems = Agent.Instance.Create(player, Logic.Option.LeftPanel.Operation, target);
        ...
        public static void EquipmentSlotClick(Player player, Option.Item clickedItem, Ability target)
        {
            ...
            var partName = Text.Agent.Instance.Get((Logic.Text.Labels)part.Type, player);
            var equippedItem = part.Content.Gets<Logic.Item>().FirstOrDefault();
