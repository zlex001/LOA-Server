    ...
    public class PlayerMonitor
    {
        ...
        public async void OnGetPlayerList(params object[] args)
        {
            ...
            try
            {
                ...
                var allPlayers = Logic.Database.Agent.Instance.Content.Gets<Logic.Database.Player>().ToList();
                var onlinePlayerIds = Logic.Agent.Instance.Content.Gets<Logic.Player>()
                    .Select(p => p.Id)
                    .ToHashSet();
                ...
                var playerList = allPlayers.Select(dbPlayer =>
                {
                    ...
                    var onlinePlayer = isOnline ? Logic.Agent.Instance.Content.Get<Logic.Player>(p => p.Id == dbPlayer.Id) : null;
        ...
        public async void OnGetPlayerDetails(params object[] args)
        {
            ...
            try
            {
                ...
                var onlinePlayer = Logic.Agent.Instance.Content.Get<Logic.Player>(p => p.Id == playerId);
                var dbPlayer = Logic.Database.Agent.Instance.Content.Get<Logic.Database.Player>(p => p.Id == playerId);
                ...
                var basic = new
                {
                    ...
                    gender = dbPlayer.grade.ContainsKey(Logic.Life.Attributes.Con) ? "Unknown" : "Unknown"
                };

                var onlineParts = isOnline ? onlinePlayer.Content.Gets<Logic.Part>().ToList() : new List<Logic.Part>();
                var attributes = new
                {
                    ...
                    atk = isOnline ? (int)onlinePlayer.Atk : dbPlayer.grade.GetValueOrDefault(Logic.Life.Attributes.Atk, 0),
                    def = isOnline ? (int)onlinePlayer.Def : dbPlayer.grade.GetValueOrDefault(Logic.Life.Attributes.Def, 0),
                    agi = isOnline ? (int)onlinePlayer.Agi : dbPlayer.grade.GetValueOrDefault(Logic.Life.Attributes.Agi, 0),
                    ine = isOnline ? (int)onlinePlayer.Ine : dbPlayer.grade.GetValueOrDefault(Logic.Life.Attributes.Ine, 0),
                    con = isOnline ? (int)onlinePlayer.Con : dbPlayer.grade.GetValueOrDefault(Logic.Life.Attributes.Con, 0)
                };
        ...
        public async void OnGetPlayerHistory(params object[] args)
        {
            ...
            try
            {
                ...
                var historyData = Logic.Database.Agent.Instance.QueryPlayerHistory(playerId, startDate, endDate);
