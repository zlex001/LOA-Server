...
namespace Domain
{
    public static class Condition
    {
        ...
        public static bool Check(Logic.Ability element, List<string> conditions, object[] eventArgs)
        {
        ...
        private static bool Check(Logic.Ability element, string condition, object[] eventArgs)
        {
        ...
        {
            if (args.Length < 2 || !(args[0] is Logic.Life) || !(args[1] is string))
                return false;

            Logic.Life life = args[0] as Logic.Life;
        ...
        {
            Logic.Ability element = (Logic.Ability)args[0];
        ...
        {
            if (args.Length < 2 || !(args[0] is Logic.Ability) || !(args[1] is string))
            {
                return false;
            }
            ...
            // 检查元素是否有身体部位组件
            if (!(element is Logic.Life life))
            {
                return false;
            }

            // 将中文部位名称转换为英文枚举值
            Logic.Part.Types partType;
            if (Enum.TryParse(requiredPart, true, out partType))
            {
                // 检查生命体是否有指定类型的身体部位
                return life.Content.Has<Logic.Part>(p => p.Type == partType);
            }
            else
            {
                return false;
        ...
        {
            if (args.Length < 3 || !(args[0] is Logic.Ability) || !(args[1] is string) || !(args[2] is object[]))
            {
                return false;
            }
            ...
            if (!(targetObj is Logic.Character character))
            {
                return false;
            }

            int targetId;
            if (character is Logic.Life life && life is not Logic.Player)
            {
                targetId = life.Config.Id;
            }
            else if (character is Logic.Item item)
            {
        ...
        private static bool CheckHandle(params object[] args)
        {
            ...
            Logic.Item handleItem = Exchange.Agent.GetHandleItem(life);
            if (handleItem == null)
                return false;
