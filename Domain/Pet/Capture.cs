...
namespace Domain.Pet
{
    public class Capture
    {
        public static bool TryCapture(Player player, Life animal, Item item)
        {
            ...
            {
                Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CaptureNotInterested)], 
            ...
            {
                Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CompanionSlotFull)], 
            ...
            if (huntSkill == null)
            {
                Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CaptureNeedSkill)], 
                    ("sub", player), ("obj", animal));
                return true;
            }
            ...
            if (success)
            {
                player.Database.companions.Add(new Logic.Database.Companion
                {
                ...
                Domain.Relation.Do(player, animal, Relation.Reason.Help);
                Domain.Relation.Do(animal, player, Relation.Reason.Help);

                Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CaptureSuccess)], 
                    ("sub", player), ("obj", animal));
            }
            else
            {
                Broadcast.Instance.Local(player, [Domain.Text.Agent.Instance.Id(Logic.Text.Labels.CaptureFail)], 
                    ("sub", player), ("obj", animal), ("item", item));
            }
