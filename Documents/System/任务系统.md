...
**添加地图**（OnAddMap）是Domain.Quest监听Map添加事件的回调函数，提取Map的quests配置并调用Check处理。

**添加生物**（OnAddLife）是Domain.Quest监听Life添加事件的回调函数，提取Life的quests配置并调用Check处理。

**添加道具**（OnAddItem）是Domain.Quest监听Item添加事件的回调函数，提取Item的quests配置并调用Check处理。

**任务检查**（Check）是Domain.Quest中统一处理quests配置的核心函数，检查是否存在相同ID的Quest实例，进而调用Integrate或CreateAndIntegrate方法。

**Quest实例是否存在**（Exist）是检查Logic.Agent.Instance.Content中是否已存在指定ID的Quest实例的方法。

**集成任务**（Integrate）是将已存在的Quest实例注册监听器并添加到对象Content中的方法。

**创建并集成**（CreateAndIntegrate）是创建新的Quest实例、注册监听器并添加到对象Content中的方法。

**注册监听**（Register）是Domain.Quest继承自Basic.Manager的函数，为Quest的Trigger事件注册OnTrigger回调监听器。
...
**玩家任务处理**（Do）是Domain.Quest中玩家获得Quest时的核心处理函数，按顺序执行对话播放、副本创建、迷宫创建和奖励发放。

**播放对话**（PlayDialogues）是Domain.Quest中处理Quest对话内容的方法，根据对话数量选择不同的播放方式。

**创建副本**（CreateCopy）是Domain.Quest中根据Quest配置创建战斗副本的方法。

**创建迷宫**（CreateMaze）是Domain.Quest中根据Quest配置创建迷宫副本的方法。

**发放奖励**（GrantRewards）是Domain.Quest中处理Quest奖励的方法，包括道具、经验和技能奖励。
...
**Agent池**（Logic.Agent Pool）是所有Quest实例的全局存储容器，支持通过ID快速查找和复用Quest实例。
...
**父子关系**（Parent-Child）Quest被添加到Ability对象的Content中，但Parent始终是Logic.Agent。
