...
负责新玩家教程的状态管理和 UI 引导，遵循"零强制暂停、零遮罩、零弹窗"设计理念。

**教程入口**（Start）是由 `Login.CompleteLogin` 调用的方法，根据进度判断：None=新开始，未完成=恢复，Complete=跳过。

**完成教程**（Complete）是完成教程并传送玩家到随机城市的方法。

**获取阶段**（GetCurrentPhase）是获取玩家当前 **Phase** 的方法。

**教程判断**（IsInTutorial）是判断玩家是否在教程中的方法。

**加载进度**（LoadProgress）是从数据库加载教程进度的方法，用于数据分析。

**金矿交互**（OnItemUsed）是监听 `Item.Event.Used` 事件的回调，过滤条件为 Config.Id == 金矿。

**物品拾取**（OnItemPicked）是监听 `Item.Event.Picked` 事件的回调，过滤条件为 Phase == Collect。

**物品给予**（OnCharacterGiven）是监听 `Character.Event.Given` 事件的回调，过滤条件为接收者 == 石碑。

**蜥蜴击败**（OnLifeDie）是监听 `Life.Event.Die` 事件的回调，过滤条件为 Config.Id == 蜥蜴。

**角色前往**（OnCharacterGoTo）是监听 `Character.Event.GoTo` 事件的回调，过滤条件为目标为教程角色。

**阶段**（Phase）是标记教程当前进度的枚举。

**引导状态**（ExploreAction）是 Explore 阶段中每个目标的引导子状态。None 时等待 `Perception.FirstSeen` 事件；SeeTarget 时高亮目标并显示"前往"按钮；AtTarget 时高亮"交互"或"攻击"按钮。
...
**持久化** - 教程进度存储在 `Database.Player.record` 中：
...
3. **恢复引导**：根据 **Phase** 和完成状态发送对应的 **Guidance**
...
**遗留兼容** - 为保持向后兼容，保留了 `Step` 枚举和 `GetCurrentStep` 方法：
...
`GetCurrentStep` 会将当前 **Phase** 和 Explore 完成状态映射为对应的 Step 值。

**引导**（Guidance）是具体的提示行为，分为两种类型：**ImplicitGuidance** 和 **ExplicitGuidance**。

**隐性引导**（ImplicitGuidance）是通过空间和情境设计，让玩家自然发现操作方式，无需任何提示。设计方式：
- **唯一路径**：只有一个方向可走，玩家自然移动
- **障碍驱动**：必须解决障碍才能前进
- **资源诱导**：看到物品，自然拾取
- **视觉引导**：目标物明显、醒目

**显性引导**（ExplicitGuidance）是通过视觉提示，主动告知玩家信息或引导玩家注意。第一次触发后不再重复。表现形式：
