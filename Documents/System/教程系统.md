...
**依赖系统**：
- **Quest**：用于子任务追踪（ExploreTask 委托给 Quest.SubTask 管理）
...
**ExploreTask**（探索子任务）定义 **Explore** 阶段内的并行任务，使用 **Quest.SubTask** 进行状态追踪。

**Quest 集成**：
- 进入 Explore 阶段时，创建一个 Quest 实例
- GoldMine、Lizard 作为 Quest 的 SubTask
- 完成检查委托给 `Quest.CheckSubTaskCompletion()`
- 所有 SubTask 完成后，自动触发阶段转换

**任务定义**：

| SubTask | 完成条件 | 触发事件 |
|---------|---------|---------|
| GoldMine | 与金矿交互 | `Item.Event.Used` |
| Lizard | 击败蜥蜴 | `Life.Event.Die` |
...
|---------------|-----|------|---------|
| None | 0 | 无目标/等待发现 | 探索提示 |
| SeeTarget | 1 | 看到目标，不在同一地图 | 高亮目标 + "前往"按钮 |
| AtTarget | 2 | 到达目标所在地图 | 高亮"交互"或"攻击"按钮 |

**目标不可见时的处理**：

当未完成的目标都不在视野内时（`SelectNearestExploreTarget` 找不到候选）：
1. 设置 `CurrentAction = None`
2. 发送探索提示："继续探索周围区域"
3. 等待 `Perception.FirstSeen` 事件
4. 当目标进入视野时，重新调用 `SelectNearestExploreTarget`

这确保玩家在任何情况下都能收到引导，不会因目标暂时不可见而中断教程。
...
| `TutorialExplore` | int | 已完成的探索任务（SubTask 完成状态，由 Quest 管理） |

**与 Quest 的集成**：
- Explore 阶段的 SubTask 状态通过 Quest 实例管理
- 持久化时，将 Quest.SubTask 状态序列化为 bitmask 存储
- 恢复时，根据 bitmask 重建 Quest.SubTask 状态
