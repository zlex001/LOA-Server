...
**获取阶段**（GetCurrentPhase）是获取玩家当前 [**Phase**](#阶段phase) 的方法。
...
| Key | 类型 | 说明 |
|-----|------|------|
| `TutorialPhase` | int | 当前阶段（[**Phase**](#阶段phase) 枚举值） |
...
3. **恢复引导**：根据 [**Phase**](#阶段phase) 和完成状态发送对应的 [**Guidance**](#引导guidance)
...
`GetCurrentStep` 会将当前 [**Phase**](#阶段phase) 和 Explore 完成状态映射为对应的 Step 值。

## 引导（Guidance）

**引导**（Guidance）是具体的提示行为，分为两种类型：隐性引导（ImplicitGuidance）和显性引导（ExplicitGuidance）。

### 隐性引导（ImplicitGuidance）

**隐性引导**（ImplicitGuidance）是通过空间和情境设计，让玩家自然发现操作方式，无需任何提示。
...
**显性引导**（ExplicitGuidance）是通过视觉提示，主动告知玩家信息或引导玩家注意。第一次触发后不再重复。
...
由于视野系统的存在，玩家可能在不同地图格子上就能在 Characters 面板看到目标角色。当玩家首次"看见"一个重要目标时，应触发以下 [**ExplicitGuidance**](#显性引导explicitguidance)：
