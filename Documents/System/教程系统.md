...
flowchart TD
    ...
    TutorialStart --> CheckPhase{检测 Phase}
    CheckPhase --> |None| NewPlayer[新玩家：创建副本]
    CheckPhase --> |未完成| ResumePlayer[断线恢复：创建副本]
    CheckPhase --> |Complete| Skip([跳过教程])
    
    NewPlayer --> WalkToSand[Phase: WalkToSand<br/>引导前往沙地]
    ResumePlayer --> RestorePhase{恢复到对应阶段}
